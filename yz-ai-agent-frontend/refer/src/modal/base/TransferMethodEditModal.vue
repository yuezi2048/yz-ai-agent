          ...
          <a-row :gutter="[16, 16]">
            <a-col :span="24">
              <a-form-item
                ...
                :wrapper-col="{ span: 20 }"
              >
                <a-input v-model:value="formData.methodName" placeholder="请输入票据种类" allow-clear />
          </a-form-item>
            </a-col>
          </a-row>
          <!-- 第二行：排列顺序 -->
...
watch(() => props.modelValue, async (val) => {
  ...
  if (val && props.formData) {
    isEditing.value = !!(props.formData.methodName && props.formData.methodName.trim())
    // 保存原始数据用于重置
    originalFormData.value = {
      methodName: props.formData.methodName || '',
...
watch(() => props.formData, (newFormData) => {
  if (visible.value && newFormData) {
    if (newFormData.methodName && newFormData.methodName.trim()) {
      // 保存原始数据用于重置
      originalFormData.value = {
        methodName: newFormData.methodName || '',
