          ...
          <a-row :gutter="[16, 16]">
            <a-col :span="24">
              <a-form-item
                label="发票种类"
                name="typeName"
                :rules="[{ required: true, message: '请输入发票种类' }]"
                :label-col="{ span: 4 }"
                :wrapper-col="{ span: 20 }"
              >
                <a-input v-model:value="formData.typeName" placeholder="请输入发票种类" allow-clear />
              </a-form-item>
            </a-col>
          </a-row>
          <!-- 第二行：排列顺序 -->
...
watch(() => props.modelValue, async (val) => {
  ...
  if (val && props.formData) {
    isEditing.value = !!(props.formData.typeName && props.formData.typeName.trim())
    // 保存原始数据用于重置
...
watch(() => props.formData, (newFormData) => {
  if (visible.value && newFormData) {
    if (newFormData.typeName && newFormData.typeName.trim()) {
      // 保存原始数据用于重置
      originalFormData.value = {
        typeName: newFormData.typeName || '',
