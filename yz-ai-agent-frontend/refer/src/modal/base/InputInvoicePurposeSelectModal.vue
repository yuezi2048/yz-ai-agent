...
const validPurposeIds = computed(() => {
  return validPurposeList.value.map((p: API.InvoicePurposeItemVO) => p.id || 0).filter(id => id > 0)
})

const isSelectAll = computed(() => {
  return validPurposeIds.value.length > 0 && selectedKeys.value.length === validPurposeIds.value.length
})

const isIndeterminate = computed(() => {
  return selectedKeys.value.length > 0 && selectedKeys.value.length < validPurposeIds.value.length
})

const handleSelectAllChange = (e: any) => {
  selectedKeys.value = e.target.checked ? [...validPurposeIds.value] : []
}
...
watch(
  ...
  (val) => {
    visible.value = val
    if (val) {
      selectedKeys.value = (props.selectedPurposeIds || []).filter(id => id > 0)
    }
  },
)
...
const handleSelect = (record: API.InvoicePurposeItemVO, selected: boolean) => {
  if (!record.id) return
  if (selected) {
