<template>
  <FilterBar
    storage-key="ACCOUNTS_RECEIVABLE_PAGE_FILTER_COLLAPSED"
    @search="$emit('search')"
    @reset="$emit('reset')"
  >
    <template #filter-fields>
      <a-row :gutter="[16, 16]">
        ...
        <!-- 开票单位 -->
        <a-col :xs="24" :sm="12" :md="8" :lg="6" :xl="6">
          <a-form-item label="开票单位" class="invoice-filter-form-item">
            <a-button
              @click="$emit('open-issuer-company-modal')"
...
const handleStartDateChange = (value: Dayjs | null) => {
  ...
  // 直接同步到传入的 searchParams 引用，确保请求参数立即生效/清空
  ;(props.searchParams as any).startDate = value || undefined
}
...
const handleEndDateChange = (value: Dayjs | null) => {
  ...
  emit('update:searchParams', next)
  ;(props.searchParams as any).endDate = value || undefined
}
