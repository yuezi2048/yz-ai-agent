...
import {
  addInvoiceTypeUsingPost,
  updateInvoiceTypeUsingPost,
  deleteInvoiceTypeUsingPost,
...
import { useInvoiceTypeTable } from './useInvoiceTypeTable'

/**
 * 发票类型管理完整 Hook
 * 包含列定义、列设置、CRUD 等所有业务逻辑
 */
export function useInvoiceTypeManage() {
  // 使用基础表格 Hook
  const baseTable = useInvoiceTypeTable()
  ...
  /** 默认显示的列 */
  const defaultSelectedColumns = ['serialNo', 'typeName', 'sortOrder', 'isEnabled', 'remark']

  /** 获取存储的列设置（每次进入时使用默认值） */
  const getStoredColumns = (): string[] => {
    try {
      sessionStorage.removeItem('INVOICE_TYPE_PAGE_SELECTED_COLUMNS')
    } catch (error) {
  ...
  // 每次进入时清除列宽存储
  try {
    localStorage.removeItem('INVOICE_TYPE_PAGE_TABLE_COLUMN_WIDTHS')
  } catch (error) {
  ...
  const { resizableColumns, tableWidth, updateColumnWidth } = useAntdResizableHeader({
    ...
    columnsState: {
      persistenceKey: 'INVOICE_TYPE_PAGE_TABLE_COLUMN_WIDTHS',
  ...
  const handleColumnSettingOk = (selectedCols: string[]) => {
    ...
    try {
      sessionStorage.setItem('INVOICE_TYPE_PAGE_SELECTED_COLUMNS', JSON.stringify(selectedCols))
    } catch (error) {
  ...
  /** 表单数据 */
  const formData = ref<Partial<API.InvoiceTypeDTO>>({})
  ...
  /** 打开编辑弹窗 */
  const doEdit = (record: API.InvoiceType_) => {
    modalTitle.value = '修改-发票种类'
  ...
  /** 提交表单 */
  const handleSubmit = async (data: API.InvoiceTypeDTO, callback?: (success: boolean) => void) => {
    try {
      if (editingId.value) {
        ...
        const res = (await updateInvoiceTypeUsingPost(submitData)) as any
        if (res.data.code === 0) {
        ...
        const res = (await addInvoiceTypeUsingPost(data)) as any
        if (res.data.code === 0) {
  ...
  const handleNextEdit = async (callback?: (success: boolean) => void) => {
    try {
      ...
      let nextRecord: API.InvoiceType_ | undefined
      const current = Number(baseTable.paginationParams.current) || 1
      const pageSize = Number(baseTable.paginationParams.pageSize) || 10
  ...
  const handlePrevEdit = async (callback?: (success: boolean) => void) => {
    try {
      ...
      let prevRecord: API.InvoiceType_ | undefined
      const current = Number(baseTable.paginationParams.current) || 1
      const pageSize = Number(baseTable.paginationParams.pageSize) || 10
  ...
  const doDelete = async (id: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const res = (await deleteInvoiceTypeUsingPost({ id })) as any
          if (res.data.code === 0) {
  ...
  const typeList = ref<API.InvoiceType_[]>([])
  ...
  const fetchTypeList = async () => {
    try {
      ...
      if (res.data.code === 0) {
        ...
        typeList.value = data.map((item: { label: string; value: string }) => ({
          id: Number(item.value),
          typeName: item.label,
        })) as API.InvoiceType_[]
      }
    } catch (error) {
      console.error('获取发票种类列表失败', error)
    }
  }
  ...
  const initPageSettings = () => {
    try {
      sessionStorage.removeItem('INVOICE_TYPE_PAGE_SELECTED_COLUMNS')
      sessionStorage.removeItem('INVOICE_TYPE_PAGE_FILTER_COLLAPSED')
      localStorage.removeItem('INVOICE_TYPE_PAGE_TABLE_COLUMN_WIDTHS')
